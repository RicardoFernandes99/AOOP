...
const DATA_PAGES=["dashboard","data_distributions","data_correlations","data_scatter","data_clustering"];
const META={dashboard:["Data Explorer",""],data_distributions:["Distributions",""],data_correlations:["Correlations",""],data_scatter:["Scatterplot",""],data_clustering:["Clustering",""],overview:["Overview",""],linear_regression:["Linear Regression",""],random_forest:["Random Forest",""],decision_tree:["Decision Tree",""],neural_network:["Artificial Neural Network",""],logistic_regression:["Logistic Regression",""],predictions:["Predictions",""]};
...
const intro=(title,eyebrow,desc,chip="")=>`<div class="intro"><div><div class="eyebrow">${esc(eyebrow)}</div><h3>${esc(title)}</h3>${desc?`<p>${esc(desc)}</p>`:""}</div>${chip}</div>`;
...
const CLUSTER_PALETTE=["96,165,250","78,222,163","251,191,36","244,114,182","45,212,191","167,139,250","248,113,113","56,189,248"];
const clusterColor=(index,alpha=.72)=>`rgba(${CLUSTER_PALETTE[Math.abs(Number(index))%CLUSTER_PALETTE.length]},${alpha})`;
...
const regressionEntries=(stats)=>[["random_forest","Random Forest"],["decision_tree","Decision Tree"],["neural_network","Artificial Neural Network"],["linear_regression","Linear Regression"]].map(([key,name])=>{
  ...
  return {key,name,m:model.metrics.selected,cls:regressionToneClass(key)};
}).filter(Boolean);
...
function renderDataScatter(eda){
  ...
  return `<div class="card data-card"><div class="body"><div class="head"><div><div class="eyebrow">Scatter Explorer</div><div class="title">Feature relationship viewer</div><p class="sub">Select any numeric pair from the EDA subset and inspect the raw point cloud.</p></div>${pill("EDA","ok")}</div><div class="scatter-controls"><label class="scatter-control"><span>X</span><select id="eda-scatter-x" class="scatter-select">${columns.map(column=>option(column,defaultX)).join("")}</select></label><label class="scatter-control"><span>Y</span><select id="eda-scatter-y" class="scatter-select">${columns.map(column=>option(column,defaultY)).join("")}</select></label></div><div class="data-chart tall"><canvas id="eda-scatter-chart"></canvas></div></div></div>`;
}

function renderDataClustering(eda){
  const clustering=eda&&eda.clustering;
  const clusterSizes=Array.isArray(clustering&&clustering.cluster_sizes)?clustering.cluster_sizes:[];
  return clustering&&clustering.projection&&Array.isArray(clustering.projection.points)&&clustering.projection.points.length?`<div class="card data-card"><div class="body">${clustering.silhouette_score!=null?`<div class="data-chip-list" style="margin-bottom:18px"><span><strong>${fmt(clustering.silhouette_score,3)}</strong>Silhouette</span><span><strong>${clustering.optimal_k}</strong>K</span></div>`:""}<div class="data-chart-grid"><div><div class="head"><div><div class="eyebrow">Elbow Curve</div><div class="title">Inertia by cluster count</div><p class="sub">Recommended elbow highlighted from the saved inertia series.</p></div></div><div class="data-chart medium"><canvas id="eda-cluster-elbow"></canvas></div></div><div><div class="head"><div><div class="eyebrow">Cluster View</div><div class="title">${esc(featureLabel((clustering.projection.axes||[])[0]))} vs ${esc(featureLabel((clustering.projection.axes||[])[1]))}</div><p class="sub">Each point keeps its K-means label and is shown in the original feature space.</p></div></div><div class="data-chart medium"><canvas id="eda-cluster-map"></canvas></div></div></div>${clusterSizes.length?`<div style="margin-top:18px">${renderDataTable(clusterSizes.map(row=>({cluster:row.cluster,count:row.count,share:pct(row.share||0)})),["cluster","count","share"],{cluster:"Cluster",count:"Count",share:"Share"})}</div>`:""}</div></div>`:`<div class="card data-card"><div class="body"><div class="empty">Clustering output is unavailable in the current EDA bundle. Regenerate the dashboard bundle to include elbow and cluster projection data.</div></div></div>`;
}
...
function applyNetworkActivation(value,activation){
  if(activation==="identity")return value;
  if(activation==="relu")return Math.max(0,value);
  if(activation==="logistic")return 1/(1+Math.exp(-value));
  if(activation==="tanh")return Math.tanh(value);
  return value;
}

function predictNeuralNetworkValue(model,vector){
  let activations=vector.map(value=>Number(value));
  const weights=Array.isArray(model.weights)?model.weights:[];
  const biases=Array.isArray(model.biases)?model.biases:[];
  weights.forEach((layerWeights,layerIndex)=>{
    const layerBiases=Array.isArray(biases[layerIndex])?biases[layerIndex]:[];
    const isOutputLayer=layerIndex===weights.length-1;
    const activation=isOutputLayer?(model.out_activation||"identity"):(model.activation||"relu");
    const outputs=(layerWeights[0]||[]).map((_,outputIndex)=>{
      const bias=Number(layerBiases[outputIndex]??0);
      const weightedSum=layerWeights.reduce((sum,row,inputIndex)=>sum+(Number(row[outputIndex]??0)*Number(activations[inputIndex]??0)),bias);
      return applyNetworkActivation(weightedSum,activation);
    });
    activations=outputs;
  });
  return Number(activations[0]??0);
}
...
function predictWithModel(model,vector,payload){
  ...
  if(model.kind==="neural_network"){
    const value=predictNeuralNetworkValue(model,vector);
    return {type:"regression",value,headline:fmt(value,3),subtitle:`Predicted ${payload.target_column}`};
  }
...
function renderOverview(stats){
  const regs=regressionEntries(stats);
  ...
  const rank=(list,n)=>{const i=list.findIndex(x=>x.name===n);return [["Gold","gold"],["Silver","silver"],["Bronze","bronze"],["Tracked",""]][i]||["Tracked",""];};
  return `${intro("Observatory Overview","Portfolio View","",pill("Bundle Ready","ok"))}
  <div class="grid4">
...
function renderLogistic(model,v=model.metrics.selected){
  ...
  return `${intro("Logistic Regression","Classifier View","",pill(quality(v.test_accuracy,{excellent:.85,good:.72,ok:.6}),v.test_accuracy>=.6?"ok":"warn"))}
  <div class="grid4">
...
function initDataScatterChart(eda){
  ...
  if(canvas&&xSelect&&ySelect&&typeof Chart!=="undefined"&&records.length){
...
function initDataClusteringCharts(eda){
  const clustering=eda&&eda.clustering;
  const elbowCanvas=document.getElementById("eda-cluster-elbow");
  const clusterCanvas=document.getElementById("eda-cluster-map");
  const elbow=clustering&&clustering.elbow;
  const projection=clustering&&clustering.projection;
  if(elbowCanvas&&typeof Chart!=="undefined"&&elbow&&Array.isArray(elbow.ks)&&Array.isArray(elbow.inertia)&&elbow.ks.length===elbow.inertia.length){
    const elbowPoints=elbow.ks.map((k,index)=>({x:Number(k),y:Number(elbow.inertia[index])})).filter(point=>Number.isFinite(point.x)&&Number.isFinite(point.y));
    const recommendedIndex=elbow.ks.findIndex(k=>Number(k)===Number(clustering.optimal_k));
    const highlighted=recommendedIndex>=0?[{x:Number(elbow.ks[recommendedIndex]),y:Number(elbow.inertia[recommendedIndex])}]:[];
    initCanvasChart("eda-cluster-elbow",{type:"line",data:{datasets:[{label:"Inertia",data:elbowPoints,borderColor:"rgba(96,165,250,1)",backgroundColor:"rgba(96,165,250,.16)",fill:false,tension:.28,pointRadius:4,pointHoverRadius:6},{label:"Recommended k",data:highlighted,borderColor:"rgba(78,222,163,1)",backgroundColor:"rgba(78,222,163,.9)",showLine:false,pointRadius:6,pointHoverRadius:8}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:"#9aa5c7"}},tooltip:{callbacks:{label:(context)=>`${context.dataset.label}: k = ${chartNumber(context.parsed.x)} | inertia = ${chartNumber(context.parsed.y)}`}}},scales:{x:{type:"linear",title:{display:true,text:"k",color:"#9aa5c7"},ticks:{color:"#9aa5c7",stepSize:1,precision:0,callback:(value)=>chartNumber(value)},grid:{color:"rgba(180,197,255,.08)"}},y:{title:{display:true,text:"Inertia",color:"#9aa5c7"},ticks:{color:"#9aa5c7",callback:(value)=>chartNumber(value)},grid:{color:"rgba(180,197,255,.08)"}}}}});
  }
  if(clusterCanvas&&typeof Chart!=="undefined"&&projection&&Array.isArray(projection.points)&&projection.points.length){
    const grouped=new Map();
    projection.points.forEach(point=>{
      const key=Number(point.cluster)||0;
      if(!grouped.has(key))grouped.set(key,[]);
      grouped.get(key).push({x:Number(point.x),y:Number(point.y)});
    });
    const datasets=Array.from(grouped.entries()).sort((a,b)=>a[0]-b[0]).map(([cluster,points])=>({label:`Cluster ${Number(cluster)+1}`,data:points,backgroundColor:clusterColor(cluster,.72),borderColor:clusterColor(cluster,1),pointRadius:4,pointHoverRadius:6}));
    const centroids=Array.isArray(projection.centroids)?projection.centroids:[];
    if(centroids.length){
      datasets.push({label:"Centroids",data:centroids.map(point=>({x:Number(point.x),y:Number(point.y)})),backgroundColor:"rgba(248,250,252,.95)",borderColor:"rgba(15,23,42,1)",pointRadius:8,pointHoverRadius:9,pointStyle:"rectRot"});
    }
    const axes=Array.isArray(projection.axes)?projection.axes:[];
    const axisX=featureLabel(axes[0]||"x");
    const axisY=featureLabel(axes[1]||"y");
    const chart=new Chart(clusterCanvas,{type:"scatter",data:{datasets},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:"#9aa5c7"}},tooltip:{backgroundColor:"rgba(8,17,33,.96)",titleColor:"#dae2fd",bodyColor:"#dae2fd",borderColor:"rgba(180,197,255,.18)",borderWidth:1,callbacks:{label:(context)=>`${context.dataset.label}: ${axisX} ${chartNumber(context.parsed.x)} | ${axisY} ${chartNumber(context.parsed.y)}`}}},scales:{x:{title:{display:true,text:axisX,color:"#9aa5c7"},ticks:{color:"#9aa5c7",callback:(value)=>chartNumber(value)},grid:{color:"rgba(180,197,255,.08)"}},y:{title:{display:true,text:axisY,color:"#9aa5c7"},ticks:{color:"#9aa5c7",callback:(value)=>chartNumber(value)},grid:{color:"rgba(180,197,255,.08)"}}}}});
    chartInstances.push(chart);
  }
}
...
function renderDashboard(stats){
  ...
  const neuralSection=document.getElementById("sec-neural_network");
  if(neuralSection)neuralSection.innerHTML=stripTileComments(renderRegression(stats.models.neural_network));
  ...
  if(stats.models.neural_network)initChart(stats.models.neural_network,stats.models.neural_network.plots.test_actual_vs_predicted);
...
function renderCurrentPage(stats){
  ...
  if(page==="data_clustering")root.innerHTML=stripTileComments(renderDataClustering(eda));
  ...
  if(page==="neural_network")root.innerHTML=stats.models.neural_network?stripTileComments(renderRegression(stats.models.neural_network)):'<div class="empty">Neural network bundle missing. Run the MLflow pipeline and regenerate the dashboard assets to expose the ANN model.</div>';
  ...
  if(page==="data_clustering")initDataClusteringCharts(eda);
  ...
  if(page==="neural_network"&&stats.models.neural_network)initChart(stats.models.neural_network,stats.models.neural_network.plots.test_actual_vs_predicted);
  return true;
}
