...
def detect_elbow_k(ks: list[int], inertias: list[float]) -> int:
    if not ks:
        return 1
    if len(ks) == 1:
        return int(ks[0])
    ...
    y_values = np.asarray(inertias, dtype=float)
    start = np.array([x_values[0], y_values[0]])
    end = np.array([x_values[-1], y_values[-1]])
    ...
    for x_value, y_value in zip(x_values, y_values):
        numerator = abs(
            line_vector[1] * (x_value - start[0]) - line_vector[0] * (y_value - start[1])
        )
        distances.append(numerator / line_norm)

    elbow_index = int(np.argmax(distances))
    elbow_k = int(ks[elbow_index])
    if elbow_k == 1 and len(ks) > 1:
        return int(ks[1])
    return elbow_k
...
def build_clustering_payload(
    ...
    *,
    ...
    cluster_df = numeric_df[feature_columns].replace([np.inf, -np.inf], np.nan)
    cluster_df = cluster_df.dropna(axis=0, how="any")
    if cluster_df.empty or len(cluster_df) < 2:
        return None

    scaler = StandardScaler()
    scaled_values = scaler.fit_transform(cluster_df)

    max_cluster_count = min(max_k, len(cluster_df))
    ks = list(range(1, max_cluster_count + 1))
    inertias: list[float] = []
    for k in ks:
    ...
    optimal_k = detect_elbow_k(ks, inertias)
    cluster_model = KMeans(n_clusters=optimal_k, random_state=random_state, n_init=10)
    labels = cluster_model.fit_predict(scaled_values)
    silhouette = (
        float(silhouette_score(scaled_values, labels))
        if optimal_k > 1 and len(np.unique(labels)) > 1
        else None
    )

    x_feature = "indicador_si" if "indicador_si" in numeric_df.columns else feature_columns[0]
    if "taxa_resolucao" in numeric_df.columns:
        y_feature = "taxa_resolucao"
    else:
        y_feature = next((column for column in feature_columns if column != x_feature), x_feature)

    view_source = numeric_df.loc[cluster_df.index, :]
    cluster_view = view_source.join(pd.Series(labels, index=cluster_df.index, name="cluster"))
    view_points = [
        {
        ...
        for _, row in cluster_view.iterrows()
    ]
    view_centroids = [
        {
            "x": float(cluster_view.loc[cluster_view["cluster"] == cluster_id, x_feature].mean()),
            "y": float(cluster_view.loc[cluster_view["cluster"] == cluster_id, y_feature].mean()),
    ...
    pca_variance = None
    if min(scaled_values.shape[0], scaled_values.shape[1]) >= 2:
        projector = PCA(n_components=2, random_state=random_state)
        projector.fit(scaled_values)
        pca_variance = [float(value) for value in projector.explained_variance_ratio_.tolist()]
    ...
    cluster_sizes["share"] = cluster_sizes["count"] / float(len(cluster_df))
    ...
    return {
        ...
        "projection": {
            "axes": [x_feature, y_feature],
            "explained_variance": pca_variance,
            "points": view_points,
            "centroids": view_centroids,
        },
        "cluster_sizes": [
            {
                "cluster": f"Cluster {int(row['cluster']) + 1}",
                "count": int(row["count"]),
...
def build_eda_payload(parent_run_dir: Path, source_df: pd.DataFrame, target_column: str) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    clustering_columns = [column for column in numeric_columns if column != target_column] or numeric_columns
    clustering_payload = build_clustering_payload(
        numeric_df,
        clustering_columns,
...
def build_neural_network_summary(model) -> dict[str, object] | None:
    if not hasattr(model, "coefs_"):
        return None

    hidden_layers = getattr(model, "hidden_layer_sizes", ())
    if isinstance(hidden_layers, int):
        hidden_layer_sizes = [int(hidden_layers)]
    else:
        hidden_layer_sizes = [int(size) for size in hidden_layers]

    return {
        "hidden_layer_sizes": hidden_layer_sizes,
        "activation": str(getattr(model, "activation", "relu")),
        "out_activation": str(getattr(model, "out_activation_", "identity")),
        "n_layers": int(getattr(model, "n_layers_", len(hidden_layer_sizes) + 2)),
...
def build_prediction_model_payload(
    ...
    *,
    ...
    if model_key == "neural_network":
        summary = build_neural_network_summary(model)
        if summary is None:
            raise ValueError("The selected neural network model does not expose MLP weights.")
        ...
        return {
            ...
            "out_activation": summary["out_activation"],
            "hidden_layer_sizes": summary["hidden_layer_sizes"],
            "layer_sizes": [int(model.coefs_[0].shape[0]), *summary["hidden_layer_sizes"], int(model.n_outputs_)],
            "weights": [
                [[float(weight) for weight in row] for row in layer.tolist()]
                for layer in model.coefs_
            ],
            "biases": [[float(bias) for bias in layer.tolist()] for layer in model.intercepts_],
        }
...
def build_regression_model(
    ...
    return {
        ...
        "network": build_neural_network_summary(model),
    }
...
def main() -> None:
    ...
    model_runs = {
        ...
        "neural_network": ("Artificial Neural Network", child_runs.get("neural_network_regressor")),
    }
